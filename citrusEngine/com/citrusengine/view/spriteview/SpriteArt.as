package com.citrusengine.view.spriteview 
{
	...
	public class SpriteArt extends Sprite
	{
		...
		public function moveRegistrationPoint(registrationPoint:String):void {
			...
			}else if (registrationPoint == "bottomRight") {
				content.x = -content.width / 2;
				content.y = -content.height / 2;
			}
		...
		public function update(stateView:SpriteView):void
		{
			...
			if (_physicsComponent) {
				...
				if(_citrusObject is Box2DPhysicsObject){
				/*	x = _citrusObject.x + Box2DPhysicsObject(_citrusObject).width/2 + (-stateView.viewRoot.x * (1 - _citrusObject.parallax)) + _citrusObject.offsetX * scaleX ;
					y = _citrusObject.y + Box2DPhysicsObject(_citrusObject).height/2 + (-stateView.viewRoot.y * (1 - _citrusObject.parallax)) + _citrusObject.offsetY;
			...
			//		_citrusObject.x = x + Box2DPhysicsObject(_citrusObject).width/2;
			//		_citrusObject.y = y + Box2DPhysicsObject(_citrusObject).height/2;
